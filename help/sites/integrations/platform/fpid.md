...
L’integrazione di siti Adobe Experience Manager (AEM) distribuiti tramite AEM Publish con Adobe Experience Platform (AEP) richiede che AEM generi e mantenga un cookie FPID (First-party Device ID) univoco per poter tenere traccia in modo univoco dell’attività dell’utente.
...
Il servizio di pubblicazione di AEM ottimizza le prestazioni memorizzando nella cache le richieste il maggior numero possibile, sia nella cache CDN che in quella Dispatcher di AEM.
...
Il diagramma seguente descrive come il servizio di pubblicazione di AEM gestisce gli FPID.

![FPID e diagramma di flusso di AEM](./assets/aem-fpid-flow.png)

1. Il browser Web richiede una pagina Web ospitata da AEM. La richiesta può essere gestita utilizzando una copia della pagina web memorizzata nella cache da CDN o AEM Dispatcher.
1. Se la pagina web non può essere servita da cache CDN o Dispatcher di AEM, la richiesta raggiunge il servizio di pubblicazione di AEM, che genera la pagina web richiesta.
1. La pagina web viene quindi restituita al browser web, popolando le cache che non potevano servire la richiesta. Con AEM, prevedi che le percentuali di hit della cache CDN e AEM Dispatcher siano superiori al 90%.
...
1. Il servlet FPID personalizzato nel servizio di pubblicazione di AEM elabora la richiesta, generando un nuovo FPID quando non viene trovato alcun cookie FPID esistente o estendendo la durata di qualsiasi cookie FPID esistente. Il servlet restituisce anche l’FPID nel corpo della risposta per l’utilizzo da parte di JavaScript lato client. Fortunatamente la logica del servlet FPID personalizzato è leggera e impedisce a questa richiesta di influire sulle prestazioni del servizio AEM Publish.
...
Il codice e la configurazione seguenti possono essere distribuiti al servizio di pubblicazione di AEM per creare un endpoint che genera o estende la durata di un cookie FPID esistente e restituisce l’FPID come JSON.

### Servlet cookie FPID di pubblicazione AEM

È necessario creare un endpoint HTTP di pubblicazione AEM per generare o estendere un cookie FPID, utilizzando un servlet [Sling](https://sling.apache.org/documentation/the-sling-engine/servlets.html#registering-a-servlet-using-java-annotations-1).
...
Di seguito è riportato il codice di esempio di un endpoint servlet di AEM (disponibile tramite `HTTP GET /bin/aep/fpid`) che genera o aggiorna un cookie FPID e restituisce l&#39;FPID come JSON.
...
Infine, le richieste HTTP GET al servlet FPID personalizzato devono essere consentite tramite la configurazione `filter.any` di AEM Dispatcher.

Se questa configurazione di Dispatcher non è implementata correttamente, le richieste HTTP GET a `/bin/aep/fpid` restituiscono il valore 404.
