---
title: Estendere un componente | Guida introduttiva dell’Editor SPA di AEM e Angular
...
3. Ispeziona il file `ui.apps/src/main/content/jcr_root/apps/wknd-spa-angular/components/image/.content.xml`:
...
6. Nello switch IDE al modulo `ui.frontend`, passa a `ui.frontend/src/app/components/card`:
   ...
   ![Avvio del componente angolare](assets/extend-component/angular-card-component-start.png)
...
7. Ispeziona il file `card.component.ts`.
   ...
   Esamina i tre parametri `@Input` nella classe per `src`, `alt` e `title`. Questi sono valori JSON attesi dal componente AEM che verrà mappato sul componente Angular.
...
8. Aprire il file `card.component.html`:
   ...
   In questo esempio abbiamo scelto di riutilizzare il componente Immagine angolare esistente `app-image` semplicemente passando i parametri `@Input` da `card.component.ts`. Successivamente nell’esercitazione verranno aggiunte e visualizzate altre proprietà.
...
2. Passa al modello di pagina SPA all’indirizzo [http://localhost:4502/editor.html/conf/wknd-spa-angular/settings/wcm/templates/spa-page-template/structure.html](http://localhost:4502/editor.html/conf/wknd-spa-angular/settings/wcm/templates/spa-page-template/structure.html).
...
Per esporre infine i valori dalla finestra di dialogo del componente al componente Angular, è necessario aggiornare il modello Sling che compila il JSON per il componente `Card`. Abbiamo anche l&#39;opportunità di implementare due elementi di logica di business:
...
4. Apri `card.component.html` e aggiungi il seguente markup per visualizzare il titolo, la chiamata all’azione e l’ultima data di modifica:
   ...
   >[!NOTE]
   >
   > Puoi visualizzare il codice del componente della scheda angolare [completato qui](https://github.com/adobe/aem-guides-wknd-spa/tree/Angular/extend-component-solution/ui.frontend/src/app/components/card).

5. Distribuisci le modifiche complete ad AEM dalla radice del progetto utilizzando Maven:
