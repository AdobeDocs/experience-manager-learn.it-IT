---
title: Creare un componente personalizzato | Guida introduttiva dell’Editor SPA di AEM e Angular
...
5. Aggiorna la variabile statica `RESOURCE_TYPE` in modo che punti al componente AEM `wknd-spa-angular/components/custom-component` creato nell&#39;esercizio precedente.
   ...
   Il tipo di risorsa del componente è ciò che associa il modello Sling al componente AEM e, in ultima analisi, verrà mappato al componente Angular.
...
6. Aggiungi il metodo `getExportedType()` alla classe `CustomComponentImpl` per restituire il tipo di risorsa del componente:
   ...
   Questo metodo è necessario durante l’implementazione dell’interfaccia `ComponentExporter` e espone il tipo di risorsa che consente la mappatura al componente Angular.
...
## Aggiorna il componente Angular
...
5. Apri `custom.component.css` e aggiungi la seguente regola:
   ...
   Affinché il segnaposto di AEM Editor venga visualizzato correttamente quando il componente è vuoto, è necessario impostare `:host-context` o un altro `<div>` su `display: block;`.
