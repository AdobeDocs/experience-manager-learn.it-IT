...
+ __Causa:__ L&#39;ambiente di destinazione della conduttura si trova in una fase di transizione in cui non può accettare nuove costruzioni.
+ __Risoluzione:__ Attendete che lo stato venga risolto in uno stato di esecuzione (o di aggiornamento disponibile). Se l’ambiente viene eliminato, ricreate l’ambiente o scegliete un ambiente diverso in cui creare l’ambiente.
...
+ __Risoluzione:__ Modificate la configurazione della pipeline e selezionate di nuovo l&#39;ambiente in cui distribuire.

### Impossibile trovare il ramo Git associato alla pipeline

+ __Messaggio di errore:__ pipeline non valida: XXXXXX. Reason=Branch=xxxx non trovato nella directory archivio.
...
+ __Risoluzione:__ Create nuovamente il ramo Git mancante con lo stesso nome oppure riconfigurate la pipeline per creare da un altro ramo esistente.

## Build e unit test

![Creazione e testing di unità](./assets/build-and-deployment/build-and-unit-testing.png)

La fase Build e Unit Testing (Test unità) esegue una build (`mvn clean package`) Maven del progetto estratto dal ramo Git configurato della pipeline.
...
+ Viene utilizzata una dipendenza del cielo non disponibile su [Maven Central](https://search.maven.org/) , e il repository Maven contenente la dipendenza è:
   ...
   + Non registrato esplicitamente nel progetto `pom.xml`. Si noti che, inclusi i repository Maven è scoraggiato man mano che aumenta i tempi di costruzione.
+ I test delle unità non riescono a causa di problemi di tempo. Ciò può verificarsi quando i test di unità sono sensibili ai tempi. Un indicatore forte si basa `.sleep(..)` sul codice di prova.
...
Per risolvere i problemi di scansione del codice, scarica il rapporto in formato CSV fornito da Cloud Manager tramite il pulsante **Scarica dettagli** e controlla tutte le voci.

Per ulteriori dettagli, vedi AEM regole specifiche, consulta le regole di scansione del codice specifiche per la documentazione di Cloud Manager [personalizzate](https://docs.adobe.com/content/help/en/experience-manager-cloud-manager/using/how-to-use/custom-code-quality-rules.html)AEM.
...
+ __Causa:__ Il pacchetto all del progetto AEM contiene più pacchetti di codice, e la stessa configurazione OSGi è fornita da più pacchetti di codice, dando luogo a un conflitto; il passaggio Build Image (Genera immagine) non è in grado di decidere quale debba essere utilizzato, dando così luogo a un errore. Questo non si applica alle configurazioni di fabbrica OSGi, purché abbiano nomi univoci.
+ __Risoluzione:__ Rivedete tutti i pacchetti di codice (inclusi eventuali pacchetti di codice di terze parti) distribuiti come parte dell&#39;applicazione AEM, alla ricerca di configurazioni OSGi duplicate che risolvono, tramite modalità di esecuzione, all&#39;ambiente di destinazione. La guida del messaggio di errore &quot;imposta il flag mergeConfigurations su true&quot; non è possibile in AEM come servizio Cloud e deve essere ignorata.
...
+ __Causa:__ Uno script repoinit non è corretto. Questo potrebbe lasciare il repository in uno stato incompleto come qualsiasi script di reindirizzamento dopo che lo script non riuscito verrà eseguito contro il repository.
+ __Risoluzione:__ Controlla il quickstart locale dell’SDK AEM quando la configurazione OSGi dello script di repoinit è distribuita per determinare se e quali sono gli errori.
...
+ L&#39;applicazione di distribuzione aggiorna la versione della dipendenza principale dei componenti core nel progetto `core` (bundle OSGi)
...
`com.adobe.cq.wcm.core.components...` impossibile importare pacchetti in intervalli di versioni specifici dal `core` progetto.
...
+ __Causa:__  Il bundle OSGi dell&#39;applicazione (definito nel `core` progetto) importa classi Java dalla dipendenza di base dei componenti core, a un livello di versione diverso da quello distribuito per AEM come Cloud Service.
+ __Risoluzione:__
...
Acquisisci familiarità con [AEM come registri](./logs.md) di Cloud Service prima di eseguire il debug della distribuzione in un passaggio. Il `aemerror` registro contiene informazioni sull’avvio e la chiusura dei contenitori che potrebbero essere pertinenti per la distribuzione a problemi. Il registro disponibile tramite il pulsante Scarica registro nel passaggio Distribuisci a di Cloud Manager non è il `aemerror` registro e non contiene informazioni dettagliate relative all&#39;avvio delle applicazioni.
...
+ __Causa:__ Il codice personalizzato può eseguire operazioni, come query di grandi dimensioni o traversate di contenuti, attivate in anticipo nel bundle OSGi o nei cicli di vita dei componenti, con un significativo ritardo del tempo di avvio dei AEM.
+ __Risoluzione:__ Controllate l&#39;implementazione del codice in esecuzione nelle prime fasi del ciclo di vita di OSGi Bundle, controllate i `aemerror` registri dei servizi AEM Author e Publish in prossimità del momento dell&#39;errore (ora di accesso nel periodo GMT) come mostrato da Cloud Manager e cercate i messaggi di registro che indicano eventuali processi di log in esecuzione personalizzati.
...
+ __Causa:__ Il codice personalizzato può richiamare operazioni lunghe, come query di grandi dimensioni o traversate di contenuti, attivate in anticipo nel bundle OSGi o nei cicli di vita dei componenti, con un significativo ritardo del tempo di avvio dei AEM.
+ __Risoluzione:__ Controllate i `aemerror` registri relativi ai servizi AEM Author e Publish nel momento (ora di accesso in GMT) dell&#39;errore, come mostrato da Cloud Manager.
...
`/var` è modificabile e contiene una varietà di contenuti di runtime transitori. Inclusione `/var` in pacchetti di contenuto (ad esempio `ui.content`) distribuita tramite Cloud Manager potrebbe causare il fallimento della distribuzione.
...
1. Determinando in che modo almeno un pacchetto di contenuto che fa parte della distribuzione viene scritto in `/var`.
...
+ __Causa:__ AEM utente del servizio di replica utilizzato per distribuire pacchetti di contenuto al servizio AEM Publish non è in grado di scrivere su `/var` AEM Publish. Questo causa il fallimento della distribuzione del pacchetto di contenuti al servizio AEM Publish.
+ __Risoluzione:__ I seguenti modi per risolvere questi problemi sono elencati nell&#39;ordine di preferenza:
   1. Se le `/var` risorse non sono necessarie, rimuovete le risorse `/var` dai pacchetti di contenuto distribuiti come parte dell&#39;applicazione.
   2. Se le `/var` risorse sono necessarie, definire le strutture dei nodi utilizzando [repoinit](https://docs.adobe.com/content/help/en/experience-manager-cloud-service/implementing/deploying/overview.html#repoinit). Gli script di reindirizzamento possono essere indirizzati a AEM Author, AEM Publish o a entrambi tramite le modalità di esecuzione OSGi.
   3. Se le `/var` risorse sono richieste solo per AEM autore e non possono essere ragionevolmente modellate utilizzando il [reindirizzamento](https://docs.adobe.com/content/help/en/experience-manager-cloud-service/implementing/deploying/overview.html#repoinit), spostatele in un pacchetto di contenuto discreto, che viene installato solo su AEM Author [incorporandolo](https://docs.adobe.com/content/help/en/experience-manager-cloud-service/implementing/developing/aem-project-content-package-structure.html#embeddeds) nel `all` pacchetto in una cartella in modalità di esecuzione di AEM Author (`<target>/apps/example-packages/content/install.author</target>`).
