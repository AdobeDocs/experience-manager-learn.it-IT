...
1. Se Homebrew non è installato, installare Homebrew
   + [Installazione di homebrew in macOS](https://brew.sh/)
      + Per l&#39;homebrew in macOS è necessario [Xcode](https://apps.apple.com/us/app/xcode/id497799835) o strumenti [della riga di](https://developer.apple.com/download/more/)comando, installabili tramite il comando:
...
Se si utilizza l’opzione per l’uso in lingua ebraica, seguire le istruzioni __di installazione in lingua ebraica__ riportate di seguito. Se __non__ usate l’interfaccia in lingua ebraica, installate gli strumenti utilizzando i collegamenti specifici del sistema operativo.

## Installare Git

[Git](https://git-scm.com/) è il sistema di gestione del controllo del codice sorgente utilizzato da [Adobe Cloud Manager](https://docs.adobe.com/content/help/en/experience-manager-cloud-manager/using/requirements/source-code-repository.html), ed è quindi richiesto per lo sviluppo.
...
[Node.js](https://nodejs.org) è un ambiente di runtime JavaScript utilizzato per lavorare con le risorse front-end di un progetto secondario __ui.frontend__ di un progetto AEM. Node.js è distribuito con [npm](https://www.npmjs.com/), è il gestore pacchetti Node.js de facto, utilizzato per gestire le dipendenze JavaScript.
...
Apache Maven è lo strumento della riga di comando Java open-source utilizzato per creare AEM Progetti generati dall&#39;Archetype AEM Project Maven. Tutti i principali IDE ([IntelliJ IDEA](https://www.jetbrains.com/idea/), codice [di](https://code.visualstudio.com/)Visual Studio, [Eclipse](https://www.eclipse.org/), ecc.) con supporto Maven integrato.
...
## Configurare  CLI I/O Adobe{#aio-cli}

L&#39;interfaccia CLI [I/O di](https://github.com/adobe/aio-cli)Adobe, o `aio`, fornisce l&#39;accesso da riga di comando a una serie di servizi di Adobe , tra cui [Cloud Manager](https://github.com/adobe/aio-cli-plugin-cloudmanager) e [Asset Compute](https://github.com/adobe/aio-cli-plugin-asset-compute). L&#39;interfaccia CLI I/O  Adobe svolge un ruolo fondamentale nello sviluppo in AEM come un Cloud Service, in quanto consente agli sviluppatori di:

+ Registri di coda da AEM come servizi di Cloud Services
+ Gestione delle pipeline di Cloud Manager dall&#39;interfaccia CLI

### Installare l&#39;interfaccia CLI I/O del Adobe 

1. Assicurarsi che [Node.js sia installato](#node-js) come CLI I/O del Adobe  è un modulo npm
   + Esegui `node --version` per confermare
1. Esegui `npm install -g @adobe/aio-cli` per installare il modulo `aio` npm a livello globale

### Configurare il plug-in CLI I/O del Adobe  CLI Cloud Manager{#aio-cloud-manager}

Il plug-in di I/O Cloud Manager di  Adobe consente all&#39;interfaccia CLI di aio di interagire con  Adobe Cloud Manager tramite il `aio cloudmanager` comando.
...
### Configurare il plug-in di calcolo risorse CLI I/O del Adobe{#aio-asset-compute}

Il plug-in di I/O Cloud Manager di  Adobe consente all&#39;interfaccia CLI di aio di generare ed eseguire Asset Compute Worker tramite il `aio asset-compute` comando.

1. Esegui `aio plugins:install @adobe/aio-cli-plugin-cloudmanager` per installare il plug-in [aio Asset Compute](https://github.com/adobe/aio-cli-plugin-asset-compute).

### Configurare l&#39;autenticazione CLI I/O del Adobe 

Affinché l&#39;interfaccia CLI di I/O del Adobe  possa comunicare con Cloud Manager, è necessario creare un&#39;integrazione di Cloud Manager nella console di I/O  Adobe e ottenere le credenziali per l&#39;autenticazione.

>[!VIDEO](https://video.tv.adobe.com/v/35094?quality=12&learn=on)

1. Accedere alla [console.adobe.io](https://console.adobe.io)
...
1. Creare un nuovo programma di I/O per Adobi [o aprirne uno esistente](https://www.adobe.io/apis/experienceplatform/console/docs.html#!AdobeDocs/adobeio-console/master/projects.md)
   +  Adobe I/O Console programmi sono semplicemente raggruppamenti organizzativi di integrazioni, creare o utilizzare e programma esistente basati su come si desidera gestire le integrazioni
   + Se create un nuovo progetto, selezionate &quot;Progetto vuoto&quot; se richiesto (vs. Crea da modello)
   +  programmi di console I/O Adobe sono concetti diversi per i programmi di Cloud Manager
1. Creare una nuova integrazione API di Cloud Manager con il profilo &quot;Sviluppatore - Cloud Service&quot;
1. Per ottenere le credenziali dell&#39;account di servizio (JWT) è necessario compilare  CLI di I/O del Adobe [config.json](https://github.com/adobe/aio-cli-plugin-cloudmanager#authentication)
1. Caricare il `config.json` file nella CLI I/O del Adobe 
   + `$ aio config:set jwt-auth PATH_TO_CONFIG_JSON_FILE --file --json`
1. Caricare il `private.key` file nella CLI I/O del Adobe 
   + `$ aio config:set jwt-auth.jwt_private_key PATH_TO_PRIVATE_KEY_FILE --file`

Iniziate [a eseguire i comandi](https://github.com/adobe/aio-cli-plugin-cloudmanager#commands) per Cloud Manager tramite l&#39;interfaccia CLI I/O del Adobe .
...
__[IntelliJ IDEA](https://www.jetbrains.com/idea/)__ è un potente IDE per lo sviluppo Java. IntelliJ IDEA è dotato di due sapori, una versione comunitaria gratuita e una versione finale commerciale (pagata). La versione comunitaria gratuita è sufficiente per lo sviluppo AEM, ma l&#39;Ultimate [amplia il suo set](https://www.jetbrains.com/idea/download)di capacità.
...
__[Visual Studio Code](https://code.visualstudio.com/)__ (VS Code) è uno strumento gratuito open-source per gli sviluppatori front-end. È possibile impostare Visual Studio Code per integrare la sincronizzazione dei contenuti con AEM con l&#39;aiuto di uno strumento di Adobe , __[repo](https://github.com/Adobe-Marketing-Cloud/tools/tree/master/repo#integration-into-visual-studio-code)__.

Visual Studio Code è la scelta ideale per gli sviluppatori front-end che creano principalmente codice front-end; JavaScript, CSS e HTML. Mentre VS Code ha il supporto Java tramite [le estensioni](https://code.visualstudio.com/docs/java/java-tutorial), potrebbe mancare alcune delle funzioni avanzate fornite da più specifiche Java.
...
__[L&#39;IDE](https://www.eclipse.org/ide/)__ Eclipse è un noto IDE per lo sviluppo Java e supporta il plug-in __[AEM Developer Tools](https://eclipse.adobe.com/aem/dev-tools/)__ fornito da  Adobe, fornendo un&#39;interfaccia utente interna all&#39;IDE per l&#39;authoring e per sincronizzare il contenuto JCR con un&#39;istanza AEM locale.
