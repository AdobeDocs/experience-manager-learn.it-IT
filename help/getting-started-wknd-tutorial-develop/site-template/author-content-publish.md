...
description: Utilizza l’editor pagina in Adobe Experience Manager, AEM, per aggiornare il contenuto del sito web. Scopri come i Componenti vengono utilizzati per facilitare l’authoring. Scopri la differenza tra un ambiente AEM Author e Publish e come pubblicare le modifiche al sito live.
...
1. Accedi al servizio **Author** dell&#39;AEM utilizzato nel capitolo precedente.
...
## Aggiornamenti Publish {#publish-updates}

Gli ambienti AEM sono suddivisi tra un **servizio Author** e un **servizio Publish**. In questo capitolo sono state apportate diverse modifiche al sito nel **servizio Author**. Per consentire ai visitatori del sito di visualizzare le modifiche, è necessario pubblicarle nel **servizio Publish**.

![Diagramma di alto livello](assets/author-content-publish/author-publish-high-level-flow.png)

*Flusso di alto livello dei contenuti dall&#39;istanza di authoring a Publish*
...
**3.** i visitatori del sito vedranno le modifiche riflesse sul servizio Publish.

### Publish le modifiche

Ora pubblichiamo le modifiche.

1. Dalla schermata iniziale dell&#39;AEM, passa a **Sites** e seleziona il **sito WKND**.
...
1. In **Opzioni** lascia le impostazioni predefinite su **Publish** e pianificale per **Ora**. Fai clic su **Avanti**.
...
1. Fai clic sul pulsante **Riferimenti pubblicati**. Nella finestra di dialogo, verifica che tutto sia selezionato. Questo includerà il **modello di sito standard** e diverse configurazioni generate dal modello di sito. Fai clic su **Fine** per aggiornare.
   ...
   ![Riferimenti a Publish](assets/author-content-publish/publish-references.png)

1. Infine, seleziona la casella accanto a **Sito WKND** e fai clic su **Avanti** nell&#39;angolo in alto a destra.
1. Nel passaggio **Flussi di lavoro**, immetti un **Titolo flusso di lavoro**. Può essere qualsiasi testo e può essere utile in seguito come parte di un audit trail. Immettere &quot;Initial Publish&quot; e fare clic su **Publish**.
...
1. Un modo semplice per ottenere l&#39;URL del servizio Publish consiste nel copiare l&#39;URL dell&#39;autore e sostituire la parola `author` con `publish`. Ad esempio:

   * **URL autore** - `https://author-pYYYY-eXXXX.adobeaemcloud.com/`
   * **URL Publish** - `https://publish-pYYYY-eXXXX.adobeaemcloud.com/`

1. Aggiungere `/content/wknd.html` all&#39;URL di Publish in modo che l&#39;URL finale sia simile al seguente: `https://publish-pYYYY-eXXXX.adobeaemcloud.com/content/wknd.html`.
...
1. Navigando all’URL di Publish dovresti visualizzare il sito, senza alcuna delle funzionalità di authoring dell’AEM.
...
1. Torna al **Servizio di authoring AEM** e apporta alcune modifiche aggiuntive al contenuto nell&#39;Editor pagina.
1. Publish queste modifiche direttamente dall&#39;editor di pagine facendo clic sull&#39;icona **Proprietà pagina** > **Pagina Publish**

   ![pubblicazione diretta](assets/author-content-publish/page-editor-publish.png)

1. Torna al **Servizio Publish AEM** per visualizzare le modifiche. Probabilmente **non** vedrà immediatamente gli aggiornamenti. Questo perché il **servizio Publish per AEM** include [caching tramite un server web Apache e CDN](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/implementing/content-delivery/caching.html). Per impostazione predefinita, il contenuto di HTML viene memorizzato nella cache per circa 5 minuti.
...
1. Puoi anche trovare l’URL del servizio Publish in Cloud Manager. Passa a **Programma Cloud Manager** > **Ambienti** > **Ambiente**.
   ...
   ![Visualizza servizio Publish](assets/author-content-publish/view-environment-segments.png)
