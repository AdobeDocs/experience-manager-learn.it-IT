...
Useremo le best practice di AEM e utilizzeremo:
...
[Adobe Cloud ](https://docs.adobe.com/content/help/it/experience-manager-cloud-manager/using/introduction-to-cloud-manager.html) Manager integra l’esecuzione di test di unità e la copertura del  [codice ](https://docs.adobe.com/content/help/en/experience-manager-cloud-manager/using/how-to-use/understand-your-test-results.html#code-quality-testing) nella pipeline CI/CD per incoraggiare e promuovere le best practice per il testing di unità del codice AEM.
...
## Ispezionare le dipendenze Maven del test {#inspect-the-test-maven-dependencies}
...
Le dipendenze di test **JUnit5**, **Mockito** e **AEM Mocks** vengono aggiunte automaticamente al progetto durante la configurazione utilizzando il [Archetipo Maven AEM](project-setup.md).
...
2. Ma anche, differenziare il file di prova    Il suffisso `Test` sul file di prova dell&#39;unità, `BylineImplTest.java` è una convenzione che ci consente di:
...
In questa esercitazione viene utilizzato l&#39;ultimo approccio (in quanto abbiamo già creato un funzionamento **BylineImpl.java** in un capitolo precedente). Per questo motivo, dobbiamo rivedere e comprendere i comportamenti dei suoi metodi pubblici, ma anche alcuni dei relativi dettagli di implementazione. Questo può sembrare contrario, poiché un buon test dovrebbe preoccuparsi solo degli input e degli output, tuttavia quando si lavora in AEM, sono necessarie diverse considerazioni di implementazione da comprendere per costruire test di lavoro.

TDD nel contesto di AEM richiede un livello di esperienza ed è meglio adottato dagli sviluppatori AEM esperti nello sviluppo AEM e nel testing di unità del codice AEM.

## Impostazione del contesto di test AEM {#setting-up-aem-test-context}

La maggior parte del codice scritto per AEM si basa su API JCR, Sling o AEM, che a loro volta richiedono che il contesto di un AEM in esecuzione sia eseguito correttamente.

Poiché i test di unità vengono eseguiti nella build, al di fuori del contesto di un&#39;istanza AEM in esecuzione, non esiste un contesto simile. Per facilitare questa fase, i cookie AEM di [wcm.io](https://wcm.io/testing/aem-mock/usage.html) creano un contesto fittizio che consente a queste API di _per lo più_ agire come se fossero in esecuzione in AEM.
...
1. Crea un contesto AEM utilizzando **wcm.io&#39;s** `AemContext` in **BylineImplTest.java** aggiungendolo come estensione JUnit decorata con `@ExtendWith` al file **BylineImplTest.java**. L&#39;estensione si occupa di tutte le attività di inizializzazione e pulizia richieste. Crea una variabile di classe per `AemContext` che può essere utilizzata per tutti i metodi di test.
   ...
   * Fornisce una varietà di oggetti e assistenti comuni richiesti come oggetti SlingHttpServletRequest, una varietà di servizi Sling e AEM OSGi simmetrici come ModelFactory, PageManager, Page, Template, ComponentManager, Component, Component, TagManager, Tag, ecc.
...
1. Nel metodo `setUp(..)`, eseguito prima di ogni metodo `@Test`, definisci uno stato comune di test del modello:
   ...
   * **`addModelsForClasses`** registra il modello Sling da testare, nel contesto AEM fittizio, in modo che possa essere istanziato nei  `@Test` metodi .
