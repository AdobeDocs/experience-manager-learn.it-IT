...
| NOME FILE`_farm.any` | In prova: `/etc/httpd/conf.dispatcher.d/available_farms/`<br>Attivo: `/etc/httpd/conf.dispatcher.d/enabled_farms/`<br><br><b>Nota:</b> questi file di farm non devono essere copiati nel `enabled_farms` cartella ma utilizza `symlinks` a un percorso relativo per `available_farms/*_farm.any` file <br/>`*_farm.any` i file sono inclusi nel `conf.dispatcher.d/dispatcher.any` file. Questi file della farm padre consentono di controllare il comportamento del modulo per ogni tipo di rendering o sito Web. I file vengono creati in `available_farms` e abilitato con un `symlink` in `enabled_farms` directory.  <br/>Le include automaticamente per nome dal file `dispatcher.any` file.<br/><b>Linea di base</b> i file farm iniziano con `000_` per assicurarsi che vengano caricati per primi.<br><b>Personalizzato</b> i file farm devono essere caricati dopo avviando il loro schema numerico in `100_` per garantire il corretto comportamento di inclusione. |
