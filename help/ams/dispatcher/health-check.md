...
description: AMS fornisce uno script cgi-bin per il controllo dello stato che i load balancer cloud eseguiranno per verificare se AEM è integro e deve rimanere in servizio per il traffico pubblico.
...
Questi script consentono al load balancer che si trova di fronte allo stack di AEM di sapere quali gambe sono sane e di mantenerle in servizio.

![GIF animato che mostra il flusso di traffico](assets/load-balancer-healthcheck/health-check.gif "Passaggi del controllo di integrità")

## Verifica stato di base del load balancer

Quando il traffico del cliente arriva tramite Internet per raggiungere l’istanza di AEM, passa attraverso un load balancer
...
> `Note:` Mentre funziona, non ha un misuratore reale su se AEM è integro.  Viene verificato solo se Dispatcher (server web Apache) è in esecuzione.

## Verifica stato AMS

Per evitare di inviare traffico a un dispatcher sano che si trova di fronte a un’istanza di AEM non integra, AMS ha creato alcuni extra che valutano lo stato della gamba e non solo di Dispatcher.
...
Per indicare se AEM funziona, è necessario compilare alcune pagine di base e distribuirle.  Adobe Managed Services ha creato un pacchetto di base che contiene la pagina di test.  La pagina verifica che l’archivio sia attivo e che sia possibile eseguire il rendering delle risorse e del modello di pagina.
...
Questo è l’endpoint leggero che possiamo testare per vedere che AEM è in esecuzione.
...
Questo script, se utilizzato, verificherà e registrerà tutte le istanze in primo piano, ma restituirà un errore solo se l&#39;istanza AEM `author` non è integra
...
Questo script, se utilizzato, verificherà e registrerà tutte le istanze in primo piano, ma restituirà un errore solo se l&#39;istanza AEM `publish` non è integra

> `Note:` Tieni presente che se l&#39;istanza di AEM per l&#39;authoring non fosse integra, il dispatcher rimarrebbe in servizio per consentire il flusso di traffico verso l&#39;istanza di AEM per la pubblicazione

#### /bin/checkeither

Questo script, se utilizzato, verificherà e registrerà tutte le istanze in primo piano, ma restituirà un errore solo se l&#39;istanza di AEM `author` o `publisher` non è integra
...
Questo script, se utilizzato, verificherà e registrerà tutte le istanze in primo piano, ma restituirà un errore solo se l&#39;istanza di AEM `author` e `publisher` non è integra

> `Note:` Tieni presente che se l&#39;istanza AEM di pubblicazione o l&#39;istanza AEM di authoring non fosse integra, Dispatcher non ritirerebbe il servizio.  Ciò significa che se uno di questi non fosse integro continuerebbe a ricevere traffico e a fornire errori alle persone che richiedono risorse.

#### /bin/integro

Questo script, se utilizzato, verifica e registra tutte le istanze che si trova davanti a, ma restituirà integro indipendentemente dal fatto che AEM restituisca o meno un errore.

> `Note:` Questo script viene utilizzato quando il controllo di integrità non funziona come desiderato e consente a un override di mantenere le istanze di AEM nel load balancer.
