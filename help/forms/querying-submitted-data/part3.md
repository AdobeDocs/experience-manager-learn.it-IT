---
title: ' AEM Forms con schema e dati JSON[Part3]'
seo-title: ' AEM Forms con schema e dati JSON[Part3]'
...
Per poter eseguire una query sui dati inviati, sarà necessario memorizzare lo schema JSON associato al modulo inviato. Lo schema JSON verrà utilizzato nel generatore di query per creare la query.

Quando viene inviato un modulo adattivo, viene verificato se lo schema JSON associato si trova nel database. Se lo schema JSON non esiste, recupereremo lo schema JSON e lo archivieremo nella tabella appropriata. Associamo inoltre il nome del modulo allo schema JSON. La schermata seguente mostra la tabella in cui sono memorizzati gli schemi JSON.
...
>[!NOTE]
>
>Durante la creazione di un modulo adattivo è possibile utilizzare lo schema JSON presente nella directory archivio o caricare uno schema JSON. Il codice riportato sopra funzionerà per entrambi i casi.
...
* Crea modulo adattivo basato su schema JSON
* Se il modulo viene inviato per la prima volta, nel database viene memorizzato lo schema JSON associato al modulo.
* I dati associati del modulo adattivo vengono memorizzati nel database.

I passaggi successivi consisterebbero nell&#39;utilizzare QueryBuilder per visualizzare i campi da cercare in base allo schema JSON
