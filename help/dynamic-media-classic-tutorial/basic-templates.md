...
- Ulteriori informazioni sui [modelli di base](https://docs.adobe.com/content/help/en/dynamic-media-classic/using/template-basics/quick-start-template-basics.html).
- Scoprite come creare un modello [](https://docs.adobe.com/content/help/en/dynamic-media-classic/using/template-basics/creating-template.html#creating_a_template)di base.
...
1. In Dynamic Media Classic, accedete a **Genera > Funzioni di base** dei modelli. Potete scegliere se non selezionare nulla oppure iniziare selezionando un’immagine che diventerà il primo livello del modello.
...
5. Trascinate i livelli di immagine desiderati. Se lo desiderate, aggiungete anche gli effetti dei livelli. Consultate l’argomento _Aggiunta di effetti_ livello, di seguito.
6. Fate clic su **Salva**, scegliete una posizione e assegnate un nome al modello. A questo punto potete visualizzare l’anteprima, ma il modello sarà esattamente come un’immagine Photoshop appiattita — non è ancora modificabile.
...
Ulteriori[informazioni sull’aggiunta di effetti](https://docs.adobe.com/content/help/en/dynamic-media-classic/using/template-basics/creating-template.html#using-shadow-and-glow-effects-on-layers)livello.
...
1. Selezionate l’opzione **Attivato** accanto al nome di ciascuna proprietà da convertire in parametro. Viene visualizzato un nome di parametro predefinito. È possibile aggiungere parametri solo alle proprietà che sono state modificate dallo stato predefinito.

   - Ad esempio, se aggiungete un livello e lo mantenete nella posizione xy predefinita pari a 0,0, Dynamic Media Classic non espone una proprietà **Position** . Per correggere il problema, spostate il livello di almeno un pixel. Ora Dynamic Media Classic espone **Posizione** come proprietà parametrizzabile.
   - Per aggiungere un parametro alla proprietà show/hide (che attiva e disattiva il livello), fate clic sull’icona **Mostra** o **Nascondi livello** per disattivare il livello (potete riattivarlo se lo desiderate). Dynamic Media Classic a questo punto espone una proprietà **Nascondi** che può essere parametrizzata.
...
Ulteriori informazioni sui parametri dei [modelli](https://docs.adobe.com/content/help/en/dynamic-media-classic/using/template-basics/creating-template-parameters.html).
...
1. Fate clic sul pulsante **dello strumento testo** nella parte superiore della pagina di creazione.
...
5. Fate clic su **Applica** per visualizzare l&#39;effetto delle modifiche nella finestra dell&#39;area di lavoro. È necessario fare clic su **Applica**. In caso contrario, le modifiche verranno perdute.
6. Al termine, fate clic su **Chiudi**. Se desiderate tornare alla modalità di modifica, fate doppio clic sul livello di testo per riaprire l’Editor di testo.
...
1. Per aggiungere un parametro, evidenziate prima il valore da modificare e fate clic sul pulsante **Aggiungi parametro** . Assicurarsi di selezionare solo i valori per i comandi e non l&#39;intero comando stesso. Ad esempio, per impostare un parametro per il nome del font nell&#39;esempio RTF precedente, evidenzierei solo &quot;Poor Richard&quot; e aggiungerei un parametro, ma non il &quot;\f0&quot;. Quando fate clic su **Aggiungi parametro** , questo viene visualizzato nell&#39;elenco seguente e il valore del parametro viene visualizzato in rosso nel RTF mentre è ancora selezionato. Per rimuovere un parametro, fate clic sulla casella di controllo accanto a **Attivato** per disattivarlo e questo scomparirà.
...
1. Fate clic su **Chiudi** per uscire dalla schermata Parametri. Quindi premete **Salva** per salvare il modello. Se avete finito di apportare modifiche, premete **Chiudi** per uscire dalla pagina Funzioni di base dei modelli.
...
>[!TIP]
>
>Per aggiungere parametri al colore del testo, aggiungete separatamente i parametri per rosso, verde e blu. Ad esempio, se il RTF è `\red56\green53\blue46`, per i valori 56, 53 e 46 si aggiungerebbero parametri di colore rosso, verde e blu. Nell’URL, il colore viene modificato chiamando tutti e tre: `&$red=56&$green=53&$blue=46`.

Scoprite come [creare parametri](https://docs.adobe.com/content/help/en/dynamic-media-classic/using/template-basics/creating-template-parameters.html#creating-dynamic-text-parameters)di testo dinamici.
...
Ulteriori informazioni sulla [pubblicazione del modello](https://docs.adobe.com/content/help/en/dynamic-media-classic/using/template-basics/publishing-templates.html).
...
![image](assets/basic-templates/sandals-without-with-parameters.png)_Esempi di un modello senza impostare i parametri (sopra) e con i parametri (sotto)._
